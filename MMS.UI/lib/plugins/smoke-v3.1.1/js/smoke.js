  ...
  $.fn.smkValidate = function() {
    ...
    if (languaje === undefined) {
      languaje = {
        ...
        textEmail: "Please provide a valid email",
        ...
        textSPassStrong: "Please specify a secure password",
    ...
    var inputs = this.is(":input")
      ? $(this)
      : $(":input:not(:button):not(:disabled):not(.novalidate)", this);
      ...
      if (value !== "") {
        ...
        if (type === "number" || smkType === "number") {
          ...
          if (!numberRegex.test(value)) {
            ...
            if (
              typeof smkMin !== "undefined" ||
              typeof smkMax !== "undefined"
            ) {
              if (value < smkMin || value > smkMax) {
                ...
                var textNumberRange = $.smokeCustomizeMsg(
                  languaje.textNumberRange,
                  arrayTextNumberRange
                );
                // Se agrega el mensaje de error
        ...
        if (smkType === "currency") {
          ...
          var currencyRegex =
            smkDecimalSeparator === "," && smkThousandSeparator === "."
        ...
        if (
          ...
          if (minlength === maxlength) {
            if (value.length != minlength && value.length != maxlength) {
              ...
              var textLength = $.smokeCustomizeMsg(
                languaje.textLength,
                maxlength
              );
              // Se agrega el mensaje de error
            ...
            if (value.length < minlength || value.length > maxlength) {
              ...
              // Se personaliza el mensaje de error
              var textRange = $.smokeCustomizeMsg(
                languaje.textRange,
                arrayTextRange
              );
              // Se agrega el mensaje de error
    ...
    this.find(".form-group.has-feedback.has-error")
      .first()
      .find(":input:not(:button):not(:disabled):not(.novalidate)")
      .first()
      .focus();
  ...
  */
  $.smkAddError = function(obj, message) {
    ...
    var ico =
      ...
      icon +
      ' form-control-feedback smk-error-icon"></span>';
  ...
  */
  ...
  $.smkAlert = function(options) {
    ...
    if (!$("body > ." + smk_alert_content_class).length) {
      $("body").append(
        '<div class="smk-alert-content ' + smk_alert_content_class + '"></div>'
      );
    }
    ...
    var obj = $(
      '<div class="alert alert-dismissable ' +
        settings.type +
        ' smk-alert"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><span class="glyphicon ' +
        settings.icon +
        '"></span><p>' +
        settings.text +
        "</p></div>"
  ...
  $.smkConfirm = function(options, callback) {
    ...
    $("body").append(
      '<div class="smk-confirm-back"><div class="panel panel-default smk-confirm" tabindex="1"><div class="panel-body">' +
        settings.text +
        '</div><div class="panel-footer text-right"><a class="btn btn-default btn-sm smk-cancel" href="#" >' +
        settings.cancel +
    ...
    $(".smk-confirm")
      ...
      .on("keydown", function(e) {
        if (e.which === 27) {
          ...
          if (!$(".smk-accept").is(":focus")) $(".smk-accept").click();
        }
      });
  ...
  $.smkPrompt = function(options, callback) {
    ...
    // Se agrega el panel de confirmacion en el body
    $("body").append(
      '<div class="smk-confirm-back"><div class="panel panel-default smk-prompt" tabindex="1"><div class="panel-body"><div class="form-group"><label for="smkPromptInput">' +
        settings.text +
        '</label><input class="form-control" id="smkPromptInput" autocomplete="off" type="text" value="' +
        settings.defaultValue +
        '"></div></div><div class="panel-footer text-right"><a class="btn btn-default btn-sm smk-cancel" href="#" >' +
        settings.cancel +
        '</a> <a class="btn btn-primary btn-sm smk-accept" href="#">' +
        settings.accept +
        "</a></div></div></div>"
  ...
  $.fn.smkShowPass = function(options) {
    ...
    var btnShow =
      '<span class="glyphicon glyphicon-eye-open smk-btn-show-pass" aria-hidden="true"></span>';
  ...
  $.smkProgressBar = function(options) {
    ...
    if (settings.status == "start") {
      ...
      progressbar +=
        '<div class="progress"><div class="progress-bar" role="progressbar" aria-valuenow="" aria-valuemin="0" aria-valuemax="100" style="width: 0%;"><span class="sr-only">0% Complete</span></div></div>';
      progressbar +=
        '<div class="smk-progressbar-content">' + settings.content + "</div>";
      progressbar += "</div>";
      ...
      $(".smk-progressbar .progress-bar").css(
        "background-color",
        settings.barColor
      );
      ...
      $(settings.element + " .smk-progressbar .progress .progress-bar").width(
        50 + Math.random() * 30 + "%"
      );
    } else if (settings.status == "end") {
      // Se completa el progreso de carga
      $(settings.element + " .smk-progressbar .progress .progress-bar")
        .width("110%")
        .delay(200, function() {
