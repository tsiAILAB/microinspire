...
(function($) {
  ...
  var numeric_data_scale = 0; // 0 decimal places by default
  ...
  var calculateObjectValue = function(self, name, args, defaultValue) {
    ...
    if (typeof name === "string") {
      if (name == "numberFormat") {
        var data_scale = numeric_data_scale;
        numeric_data_scale = 0;

        return parseFloat(defaultValue).numberFormat(
          data_scale == 0 ? false : true,
          data_scale
        );
      }
  ...
  BootstrapTable.prototype.onSort = function(event) {
    ...
    if (this.options.sortName === $this.data("field")) {
      ...
      if (this.options.rememberOrder) {
        ...
        this.options.sortOrder = this.columns[
          this.fieldsColumnsIndex[$this.data("field")]
        ].order;
      }
    }
  ...
  BootstrapTable.prototype.initPagination = function() {
    ...
    //Custom Code
    if (this.options.showPaginationDetail !== false) {
      html.push(
        sprintf(
  ...
  BootstrapTable.prototype.onPageNumber = function(event) {
    event.preventDefault();
    if ($(event.currentTarget).text() === "...") return;
  ...
  BootstrapTable.prototype.initRow = function(item, i, data, parentDom) {
    ...
    $.each(this.header.fields, function(j, field) {
      ...
      if (column.scale != undefined) {
        numeric_data_scale = column.scale;
      }
  ...
  BootstrapTable.prototype.initBody = function(fixedScroll) {
    ...
    this.$selectItem.off("click").on("click", function(event) {
      ...
      //Custom Code
      if (
        that.options.onBeforeCheckUncheck &&
        !that.options.onBeforeCheckUncheck(row, $this)
      ) {
